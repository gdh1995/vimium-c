...
(function() {
  "use strict";
  var HUD, Tween, currentCompletionKeys, singleKeyCommands //
...
href='https://github.com/philc/vimium#release-notes'>what's new</a>).<a class='vimB vimI vimL vimHUDClose'>&#215;</a>";
  ...
  requestHandlers = {
    ...
    refreshKeyMapping: function(response) {
      currentCompletionKeys = singleKeyCommands = response.singles;
      validFirstKeys = response.firsts;
      keyQueue = "";
    },
    refreshCompletionKeys: function(response) {
      var ref = response.completions;
      currentCompletionKeys = ref ? singleKeyCommands.concat(ref) : singleKeyCommands;
    ...
    executePageCommand: function(request) {
      ...
      currentCompletionKeys = singleKeyCommands;
      keyQueue = "";
    },
  ...
  settings.addEventListener("load", function(response) {
    ...
    singleKeyCommands = response.singles;
    validFirstKeys = response.firsts;
    var ref = response.completions;
    currentCompletionKeys = ref ? singleKeyCommands.concat(ref) : singleKeyCommands;
    keyQueue = response.keyQueue;
