...
(function() {
  ...
  LinkHints = {
    ...
    activateMode: function(mode) {
      ...
      hintMarkers = this.getVisibleClickableElements().map(this.createMarkerFor.bind(this));
    ...
    setOpenLinkMode: function(mode) {
      ...
      if (this.mode === OPEN_IN_NEW_BG_TAB || this.mode === OPEN_IN_NEW_FG_TAB || this.mode === OPEN_WITH_QUEUE) {
        ...
        return this.linkActivator = function(link) {
          return chrome.runtime.sendMessage({
            handler: "copyToClipboard",
            data: link.innerText || link.innerHTML.replace(/<.*?>/g, '')
          });
        };
        ...
        return this.linkActivator = function(link) {
          return setTimeout(DomUtils.simulateClick.bind(DomUtils, link), 100);
        };
      }
    },
    ...
    showMarker: function(linkMarker, matchingCharCount) {
      ...
      for (j = 0, _ref = linkMarker.childNodes; j < _ref.length; ++j) {
        if (j < matchingCharCount) {
          _results.push(_ref[j].classList.add("matchingCharacter"));
        } else {
          _results.push(_ref[j].classList.remove("matchingCharacter"));
        }
      }
      return _results;
    },
  ...
  alphabetHints = {
    ...
    shuffleHints: function(hints, characterSetLength) {
      var bucket, buckets, hint, result, i, _len;
      buckets = (function() {
        var _results = [], i = 0;
        for (; i < characterSetLength; ++i) {
          _results.push([]);
        }
        return _results;
      })();
      ...
      result = [];
      for (i = 0, _len = buckets.length; i < _len; ++i) {
        bucket = buckets[i];
