var classification = {
	...
	containerHeight : 0,
	changeSwitch : true,
	minClassificationSwitch : false,
	minClassificationTargetObj : '',
	classifications : PDI.get("classifications"),
	defaultDialBoxes : {
		normal : [{
	...
	showMinClassification : function () {
		...
		if (!_move && !_edit && $('.dialog-visible').length == 0) {
			...
			$('.minClassificationContainer').css({
				"left" : Math.round(_left + ((DBOX.QWidth + DBOX.QSpacing - ((self.classifications.length + 1) * (DBOX.QWidth - 26) + (self.classifications.length + 2) * 10)) / 2)) + "px"
			});
	...
	measurement : function () {
		...
		self.content.find(".classificationList").css({
			"width" : self.containerWidth + "px",
			"marginLeft" : "-" + parseInt(self.containerWidth / 2) + "px",
			"height" : self.containerHeight + "px",
			"marginTop" : "-" + parseInt(self.containerHeight / 2) + "px"
		})
	},
				...
				if (self.content.find(".classificationList").hasClass("edit")) {
					...
					if (_index >= 0 && _cId != "") {
						if (isContainsClass(e.target, "classificationDel")) {
							...
							if (self.isClear(2)) {
								self.content.find(".classificationLogoList").css({
									"left" : classificationObj.get(0).offsetLeft + self.content.find(".classificationList").get(0).offsetLeft - 22 - 300 + parseInt(self.width / 2) + "px",
									"top" : classificationObj.get(0).offsetTop + self.content.find(".classificationList").get(0).offsetTop + 10 - 120 + "px"
								});
								var logoNo = 0;
