		...
		weather.prototype = {
					...
					self.getWeatherData(function (data) {
						try {
							...
							self.weather = {
								...
								temp : weahterInfo.weatherinfo.temp1,
			...
			initWeatherApp : function () {
				...
				if (DBOX.width >= 180 && DBOX.height >= 90) {
					self.container.append($('<div class="weatherApp"><div class="preview"></div><div class="temp">' + self.weather.temp + '</div><div class="address">' + self.city + '</div><div class="weather">' + self.weather.text + '</div><div class="lunarCalendar">' + self.calendar.date + getI18nMsg('dateUnit') + " [" + self.calendar.weekDay + PDI.get("setup", "lunarCalendar") + ']</div></div>').css({
				...
				weatherImageUrl = urlImg + 'myapp/weather/img/m/' + self.getWeatherImage(self.weather.images[0], self.weather.images[1]);
				...
				if (cityID != "") {
					...
					$.ajax({
						...
						success : function (result) {
							...
							if (typeof result == 'object' && result.city) {
								$.ajax({
									url : urlImg + "tianqi/city.php?city=" + result.city + "&t=" + t,
									success : function (data) {
										if (typeof data == 'string' && data.substr(0, 5) != 'ERROR') {
											self.cityID = data;
											self.dateline = 0;
