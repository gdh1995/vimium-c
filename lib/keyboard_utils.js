...
KeyboardUtils = {
  ...
  getKeyChar: function(event) {
    ...
    if (keyIdentifier = this.correctionMap[keyIdentifier.substring(4)]) {
      return keyIdentifier[event.shiftKey ? 1 : 0];
    }
    // TODO: .keyIdentifier ? .keyCode
    keyIdentifier = String.fromCharCode(parseInt(event.keyIdentifier.substring(2), 16));
