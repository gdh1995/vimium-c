...
var Utils = {
  ...
  convertToUrl: function(string) {
    if (string.substring(0, 11).toLowerCase() == "javascript:") {
      if (string.indexOf('%', 11) > 0 && !this._jsDelims.test(string)) {
        if (!this._urlEscapeRegex.test(string)) {
  ...
  decodeURLPart: null,
  decodeEachURLPart: null
};

(function() {
  var decode = window.decodeURIComponent, percentRegex = /(?=%)/;
  Utils.decodeURLPart = function(url) {
    try {
      url = decode(url);
    } catch (e) {
    }
  ...
  Utils.decodeEachURLPart = function(url) {
    var arr = url.split(percentRegex), i = arr.length, e;
    for (; ; ) {
      try {
        while (0 <= --i) {
          arr[i] = decode(arr[i]);
