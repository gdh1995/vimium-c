  ...
  convertToUrl: function(string) {
    ...
    if (index > 0 && string.startsWith("javascript:")) {
      ...
      if (string.startsWith("file:") || string.startsWith("chrome:")) {
        ...
        index = string.indexOf('/', index0);
        host = string.substring(index0, index !== -1 ? index : undefined);
        expected = 0;
      }
    } else if (this.hasChromePrefix(string)) {
      type = 0;
    } else {
      host = (index = string.indexOf('/')) !== -1 ? string.substring(0, index) : string;
    }
    if (type >= 0) {}
    ...
    else if (this._nonENDomainRegex.test(host = host.substring(index + 1))) {
      type = this._nonENTlds.indexOf(host) !== -1 ? expected : 2;
    } else if (string = this._tlds[host.length]) {
      type = string.indexOf(host) !== -1 ? expected : 2;
    } else {
      type = 2;
    }
