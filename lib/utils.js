...
(typeof exports !== "undefined" && exports !== null ? exports : window).Utils = {
  ...
  isRefusingIncognito: function(url) {
    ...
    return url.startsWith('chrome-extension://') || !this.hasOrdinaryUrlPrefix(url);
  },
  _urlRegex: /^(?:[^:]+(?::[^:]+)?@)?([^:]+|\[[^\]]+\])(?::\d+)?$/,
  ...
  convertToUrl: function(string) {
    ...
    return type === 0 ? oldString : type === 1 ? ("http://" + oldString)
      : this.createSearchUrl(Settings.get("searchUrl"), oldString.split(' '));
  },
  ...
  createSearchUrl: function(baseUrl, query) {
    var str, _Str = query.join(' ');
    if (baseUrl.indexOf('%s')) {
      str = query.map(encodeURIComponent).join('+');
    }
    if (baseUrl.indexOf('%S')) {
      _Str = query.join(' ');
    }
    baseUrl = baseUrl.replace(this._searchWordRegex, function(s) {
      return (s === "%s") ? str : _Str;
    });
    return baseUrl.trim();
  },
