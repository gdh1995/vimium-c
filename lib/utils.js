...
(function() {
  ...
  Utils = {
    ...
    hasChromePrefix: function(url) {
      var _ref, _i, _len;
      if (0 < url.indexOf(":")) {
        for (_i = 0, _ref = Utils._chromePrefixes, _len = _ref.length; _i < _len; _i++) {
    ...
    hasFullUrlPrefix: function(url) {
      return Utils._urlPrefix.test(url);
    },
    createFullUrl: function(partialUrl) {
      if (Utils._urlPrefix.test(partialUrl)) {
        return partialUrl;
      } else {
    ...
    isUrl: function(str) {
      ...
      if (Utils._urlPrefix.test(str)) {
        return true;
      }
      match = Utils._urlRegex.exec((str.split('/'))[0]);
      if (!match) {
        return false;
      }
      hostName = match[3];
      if (Utils._specialHostNames.indexOf(hostName) >= 0) {
        return true;
      }
      ...
      if (dottedParts.length > 1) {
        ...
        if ((2 <= (_ref = lastPart.length) && _ref <= 3) || Utils._longTlds.indexOf(lastPart) >= 0) {
          return true;
        }
      }
      if (Utils._ipRegex.test(hostName)) {
        return true;
      }
      return false;
    },
    ...
    createSearchUrl: function(baseUrl, query) {
      return baseUrl.replace(Utils._searchWordRegex, encodeURIComponent(query));
    },
    ...
    hasUpperCase: function(s) {
      return Utils._upperRegex.test(s);
    }
  };
