...
var DomUtils = {
  ...
  createElement: function(tagName) {
    ...
    this.createElement = node instanceof HTMLElement
      ? document.createElement.bind(document)
      : document.createElementNS.bind(document, "http://www.w3.org/1999/xhtml");
    return node.style ? node : this.createElement(tagName);
  },
  ...
  isNotInlineZeroHeight: function(element) {
    ...
    return style.fontSize !== "0px" || !style.display.startsWith("inline");
  },
  ...
  getVisibleClientRect: function(element, el_style) {
    ...
    for (_i = 0, _len = arr.length; _i < _len; _i++) {
      ...
      if (rect0.width > 0 && rect0.height > 0) {
        if (isVisible == null) {
          isVisible = (el_style || window.getComputedStyle(element)).visibility === 'visible';
        }
        if (isVisible) {
      ...
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        ...
        if (style.float !== 'none' ||
          ...
          if (notInline || !style.display.startsWith("inline")) {
            continue;
          }
        }
        if (cr = this.getVisibleClientRect(_ref[_j], style)) {
          return cr;
        }
      }
      style = null;
    ...
    // if element's display is not "none", then _len > 0, so isVisibile has been assigned
    if (isVisible) {
      for (; _i < _len; _i++) {
        rect0 = arr[_i];
        cr = VRect.cropRectToVisible(rect0.left, rect0.top, rect0.right, rect0.bottom);
