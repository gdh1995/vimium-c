...
var VDom = {
  ...
  prepareCrop: function() {
    ...
    VRect.cropRectToVisible = function(left, top, right, bottom, margin) {
      var cr;
      if (top > ihs || bottom < 3) {
        return null;
      }
      ...
      return (cr[3] - cr[1] >= margin && cr[2] - cr[0] >= margin) ? cr : null;
    };
    this.prepareCrop();
  },
  getVisibleClientRect: function(element, el_style) {
    ...
    for (_i = 0, _len = arr.length; _i < _len; _i++) {
      ...
      if (rect.width > 0 && rect.height > 0) {
        if (cr = VRect.cropRectToVisible(rect.left, rect.top, rect.right, rect.bottom, 3)) {
          if (isVisible == null) {
  ...
  isInDocument: function(element) {
    ...
    while (parent = element.parentNode) { element = parent.host || parent; }
...
VRect = {
  ...
  SubtractSequence: function (rect1) { // rect1 - rect2
    var rect2 = this[1], a = this[0], x0, x1, x2, x3, y0, y1, y2, y3;
    x1 = Math.max(x0 = rect1[0], rect2[0]), x2 = Math.min(x3 = rect1[2], rect2[2]);
    y1 = Math.max(y0 = rect1[1], rect2[1]), y2 = Math.min(y3 = rect1[3], rect2[3]);
    // 1 2 3
    // 4   5
    // 6 7 8
