...
var DomUtils = {
  ...
  isSelectable: function(element, _name) {
    return element.isContentEditable || (_name = element.nodeName.toLowerCase()) === "textarea" || //
      (_name === "input" && this._unselectableTypes.indexOf(element.type) === -1);
  },
  isEditable: function(element, _name) {
    if (element.isContentEditable) {
      return true;
    }
    _name = element.nodeName.toLowerCase();
    if (_name === "input") {
      _name = element.type;
      return _name !== "radio" && _name !== "checkbox";
    }
    return _name === "textarea" || _name === "select";
  },
  isEmbed: function(element, _name) {
    return (_name = element.nodeName.toLowerCase()) === "embed" || _name === "object";
  },
  isFocusable: function(element, _name) {
    if (element.isContentEditable) {
      return true;
    }
    _name = element.nodeName.toLowerCase();
    ...
    return _name === "textarea" || _name === "select" || _name === "embed" || _name === "object";
  },
  isDOMDescendant: function(parent, child) {
    for (var _par; child != null; child = _par) {
      _par = child.parentNode;
      if (_par === parent) {
        return child;
      }
    }
  ...
  isSelected: function(element) {
    if (element.selectionStart != null && element.selectionEnd != null && element.selectionEnd !== 0) {
      return true;
    } else if (element.isContentEditable) {      var node, _ref = document.getSelection();
      if (_ref && (node = _ref.anchorNode)) {
        return this.isDOMDescendant(element, node) ? true : false;
      }
    }
    return false;
  },
  simulateSelect: function(element) {
    // TODO: why to use `this.isEditable`
    if (element === document.activeElement && this.isEditable(document.activeElement)) {
      handlerStack.bubbleEvent("click", {target: element});
      return;
    }
    element.focus();
    if (!this.isSelected(element)) {
      var len = element.value ? +element.value.length : -1, e;
  ...
  _defaultModifiersArray: [false, false, false, false],
  _eventSequence: ["mouseover", "mousedown", "mouseup", "click"],
  simulateClick: function(element, modifiers) {
    var eventSequence = this._eventSequence, mouseEvent, _i, _len, m;
    m = !modifiers ? this._defaultModifiersArray
    ...
    for (_i = 0, _len = eventSequence.length; _i < _len; _i++) {
      ...
      mouseEvent.initMouseEvent(eventSequence[_i], true, true, window, 1, 0, 0, 0, 0, m[0], m[1], m[2], m[3], 0, null);
  ...
  flashVRect: function(rect, time) {
    ...
    return setTimeout(this.removeNode.bind(this, flashEl), time != null ? time : this.flashLastingTime);
  },
