...
var HelpDialog = {
  render: (function(request) {
    ...
    var commandsToKey = Object.create(null), ref = CommandsData.keyToCommandRegistry, showUnbound = !!request.unbound, showNames = !!request.names;
    var command, key;
    for (key in ref) {
      command = ref[key].command;
      (commandsToKey[command] || (commandsToKey[command] = [])).push(key);
    }
    var result = Object.setPrototypeOf({
  ...
  groupHtml: (function(group, commandsToKey, showUnbound, showNames) {
    ...
    for (var _i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      var isAdvanced = command in this.advancedCommands, description = availableCommands[command][0];
  ...
  commandHtml: (function(html, isAdvanced, bindings, description, command) {
    ...
    if (description) {
      html.push('<td class="HelpTd HelpKeys">', bindings, '</td>\n\t<td class="HelpTd HelpCommandInfo">', description);
      if (command) {
        html.push('\n\t\t<span class="HelpCommandName" role="button">(', command, ")</span>\n\t");
      }
    } else {
      html.push('<td class="HelpTd HelpKeys HelpLongKeys" colspan="2">', bindings);
    }
    return html.push("</td>\n</tr>\n");
  }),
  ...
  advancedCommands: { __proto__: null,
