...
availableCommands_: {
  ...
  toggleSwitchTemp: [ "Toggle switch only on current page (use key[, value])", 1, true, kBgCmd.toggle ],
  toggleVomnibarStyle: [ "Toggle style(s) of vomnibar page (use style=dark, current)", 1, true,
  ...
  scrollRight: [ "Scroll right", 0, false, kFgCmd.scroll, { axis: "x" } ],
  scrollPxDown: [ "Scroll 1px down", 0, false, kFgCmd.scroll, { view: 1 } ],
  scrollPxUp: [ "Scroll 1px up", 0, false, kFgCmd.scroll, { count: -1, view: 1 } ],
  scrollPxLeft: [ "Scroll 1px left", 0, false, kFgCmd.scroll, { count: -1, axis: "x", view: 1 } ],
  scrollPxRight: [ "Scroll 1px right", 0, false, kFgCmd.scroll, { axis: "x", view: 1 } ],
  scrollTo: [ "Scroll to custom position", 0, false, kFgCmd.scroll, { dest: "min" } ],
  scrollToTop: [ "Scroll to the top of the page", 0, false, kFgCmd.scroll, { dest: "min" } ],
  scrollToBottom: [ "Scroll to the bottom of the page", 0, false, kFgCmd.scroll, { dest: "max" } ],
  scrollToLeft: [ "Scroll all the way to the left", 0, false, kFgCmd.scroll, { axis: "x", dest: "min" } ],
  scrollToRight: [ "Scroll all the way to the right", 0, false, kFgCmd.scroll, { axis: "x", dest: "max" } ],
  scrollPageDown: [ "Scroll a page down", 0, false, kFgCmd.scroll, { dir: 0.5, view: "viewSize" } ],
  scrollPageUp: [ "Scroll a page up", 0, false, kFgCmd.scroll, { dir: -0.5, view: "viewSize" } ],
  scrollFullPageDown: [ "Scroll a full page down", 0, false, kFgCmd.scroll, { view: "viewSize" } ],
  scrollFullPageUp: [ "Scroll a full page up", 0, false, kFgCmd.scroll, { count: -1, view: "viewSize" } ],
  reload: [ "Reload current frame (use hard)", 1, false, kFgCmd.reload ],
  reloadTab: [ "Reload N tab(s) (use hard, bypassCache)",
    /** 20 in main.ts */ 0, true, kBgCmd.reloadTab ],
  ...
  searchAs: [ "Search selected or copied text using current search engine (use copied=true, selected=true)", 1, false,
  ...
  "LinkHints.activateModeToDownloadImage": [ "Download image", 0, false,
    kFgCmd.linkHints, { mode: HintMode.DOWNLOAD_IMAGE } ],
  "LinkHints.activateModeToOpenImage": [ "Show image in a new tab (use auto=true)", 0, false,
  ...
  createTab: [ "Create new tab(s)", 20 as 0, true, kBgCmd.createTab ],
  duplicateTab: [ "Duplicate current tab for N times", 20 as 0, true, kBgCmd.duplicateTab ],
  removeTab: [ "Close N tab(s) (use allow_close, limited=null/&lt;boolean&gt;, left)",
    /** 20 in main.ts */ 0, true, kBgCmd.removeTab ],
  removeRightTab: [ "Close N-th tab on the right", 0, true, kBgCmd.removeRightTab ],
  restoreTab: [ "Restore closed tab(s)", 25 as 0, true, kBgCmd.restoreTab ],
  ...
  moveTabToNewWindow: [ "Move N tab(s) to new window (use limited=null/&lt;boolean&gt;)",
    /** 30 in main.ts */ 0, true, kBgCmd.moveTabToNewWindow ],
  ...
  togglePinTab: [ "Pin or unpin N tab(s)",
    /** 30 in main.ts */ 0, true, kBgCmd.togglePinTab ],
